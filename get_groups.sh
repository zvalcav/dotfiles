...
write()
{
	...
	# vychozi podoba konfiguracniho souboru
	{
		...
		echo ""
		echo "# nastavuje, do jake mysql databaze bude script sahat"
		echo "# user a heslo se nastavuje v ($mysqlDefaultsFile)"
...
if [ -z "$psqlUser" -o -z "$psqlDatabase" -o -z "$mysqlDatabase" -o -z "$mailNotification" ]
then
...
if ! mysql --defaults-extra-file="$mysqlDefaultsFile" "$mysqlDatabase"\
...
if ! mysql --defaults-extra-file="$mysqlDefaultsFile" "$mysqlDatabase" -e "
